...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        ...
        <a href="{{ route('panel.logs.export', request()->all()) }}" class="btn-premium flex items-center justify-center gap-2 bg-gray-900/50 border border-gray-800 text-white px-6 py-3.5 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-gray-800/50 transition-all shadow-sm">
            <i class="fa-solid fa-file-export text-xs text-green-500"></i> Exportar Dados
        </a>
    </div>
        ...
        <form action="{{ route('panel.logs') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div class="space-y-2">
                ...
                <div class="relative group">
                    <span class="absolute inset-y-0 left-0 pl-4 flex items-center text-gray-300 group-focus-within:text-green-500 transition-colors">
                        <i class="fa-solid fa-magnifying-glass text-xs"></i>
                    </span>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Ação ou usuário..." 
                        class="block w-full pl-10 pr-4 py-3.5 bg-gray-800/50 border-gray-800 rounded-2xl text-xs font-bold focus:ring-2 focus:ring-green-500 focus:bg-gray-900/50 transition-all outline-none">
                </div>
            </div>

            <div class="space-y-2">
                <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Tipo de Evento</label>
                <select name="acao" class="block w-full py-3.5 px-4 bg-gray-800/50 border-gray-800 rounded-2xl text-xs font-bold focus:ring-2 focus:ring-green-500 focus:bg-gray-900/50 transition-all outline-none appearance-none cursor-pointer">
            ...
            <div class="space-y-2">
                ...
                <input type="date" name="data" value="{{ request('data') }}" 
                    class="block w-full py-3.5 px-4 bg-gray-800/50 border-gray-800 rounded-2xl text-xs font-bold focus:ring-2 focus:ring-green-500 focus:bg-gray-900/50 transition-all outline-none">
            </div>
    ...
    <div class="bg-gray-900/50 bg-[#111827] border border-gray-800/50 rounded-3xl shadow-sm rounded-[32px] border border-gray-800 overflow-hidden shadow-sm">
                        ...
                        <tr class="hover:bg-gray-900/80 transition-colors group">
                            ...
                            <td class="px-8 py-5">
                                <div class="flex items-center gap-4">
                                    <div class="w-10 h-10 rounded-2xl bg-gray-800/50 flex items-center justify-center text-xs font-bold text-gray-400 border border-gray-800 group-hover:bg-green-900/30 group-hover:text-green-500 transition-all shadow-sm">
                                        {{ $log->user ? substr($log->user->name, 0, 1) : 'S' }}
                                    </div>
                                    <span class="text-xs font-bold text-white group-hover:text-green-500 transition-colors uppercase tracking-tight">{{ $log->user ? $log->user->name : 'System Core' }}</span>
                                </div>
                            </td>
                            <td class="px-8 py-5 text-center">
                                <span class="px-4 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-widest border
                                    {{ str_contains($log->acao, 'criado') ? 'bg-emerald-900/30 text-emerald-600 border-emerald-800' : 
                                       (str_contains($log->acao, 'excluido') ? 'bg-rose-50 text-rose-600 border-rose-100' : 'bg-green-900/30 text-green-500 border-green-800') }}">
        ...
        @if($logs->hasPages())
            <div class="p-8 border-t border-gray-800/50 bg-gray-900/30">
                {{ $logs->links() }}
            </div>
        @endif
    </div>
