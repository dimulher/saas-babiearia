...
<div class="space-y-6">
    ...
    <h1 class="text-2xl font-bold text-white">Planos de Assinatura</h1>

    <!-- Toggle mensal/anual -->
    <div class="flex items-center justify-center" x-data="{ anual: false }">
        <div class="flex items-center gap-4 bg-gray-900/50 border border-gray-700 rounded-2xl p-1.5">
            <button @click="anual = false" :class="!anual ? 'bg-gray-900 text-white' : 'text-gray-400'" class="px-5 py-2 rounded-xl text-sm font-medium transition-colors">Mensal</button>
            <button @click="anual = true" :class="anual ? 'bg-gray-900 text-white' : 'text-gray-400'" class="px-5 py-2 rounded-xl text-sm font-medium transition-colors">
